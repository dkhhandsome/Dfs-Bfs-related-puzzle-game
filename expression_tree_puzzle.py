...
class ExpressionTreePuzzle(Puzzle):
    ...
    def is_solved(self) -> bool:
        ...
        True
        """
        result = self._tree.eval(self.variables)
        ...
        for key in self.variables:
            if self.variables[key] == 0:
                for i in range(1, 10):
                    ...
                    temp = ExpressionTreePuzzle(tree, self.target)
                    temp.variables = self.variables.copy()
                    temp.variables[key] = i
                    result.append(temp)
        return result
    ...
    def fail_fast(self) -> bool:
        ...
        acc = self.extensions()
        if acc == []:
            value = self._tree.eval(self.variables)
            return value != self.target
        for tree in acc:
            if not tree.fail_fast():
                return False
        return True
