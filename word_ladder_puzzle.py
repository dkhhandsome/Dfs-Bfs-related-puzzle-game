...
class WordLadderPuzzle(Puzzle):
    ...
    def __eq__(self, other: WordLadderPuzzle) -> bool:
        ...
        True
        """
        if self.to_word == other.to_word and self.word_set == other.word_set \
                and self.from_word == other.from_word:
            return True
        return False
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        result = []
        i = 0
        while i < len(self.from_word):
            for ch in LETTERS:
                temp = self.from_word[:i] + ch + self.from_word[i + 1:]
                if temp != self.from_word and temp in self.word_set:
    ...
    def get_difficulty(self) -> str:
        ...
        """
        temp = BfsSolver()
        length = len(temp.solve(self))
        if self.to_word not in self.word_set:
            return IMPOSSIBLE
        if length <= 1:
            return TRIVIAL
        elif length == 2:
