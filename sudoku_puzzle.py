...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for i in range(len(self._grid)):
            for j in range(len(self._grid)):
                if self._grid[i][j] == " ":
                    a = self._row_set(i).union(self._column_set(j)). \
                        union(self._subsquare_set(i, j))
                    if self._symbol_set <= a:
                        return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        temp = DfsSolver()
        result1 = set(str(DfsSolver.solve(temp, self)))
        result2 = DfsSolver.solve(temp, self, seen=result1)
        return result2 is None
