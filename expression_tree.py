...
class ExprTree:
    ...
    def __str__(self) -> str:
        ...
        result = "("
        i = 0
        while i < len(self._subtrees) - 1:
            result += self._subtrees[i].__str__() + ' ' + self._root + ' '
            i += 1
        result += self._subtrees[i].__str__() + ')'
        return result
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        True
        """
        if isinstance(self._root, str) and self._root != '+' and \
                self._root != '*':
            lookup[self._root] = 0
        for sub in self._subtrees:
            sub.populate_lookup(lookup)
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    True
    """
    if values[0][0] != '+' and values[0][0] != '*':
        return ExprTree(values[0][0], [])
    result = ExprTree(values[0][0], [])
    for sub in values[1]:
        if sub == "+" or sub == "*":
            temp = construct_from_list([[sub], values[1:]])
            result.append(temp)
        else:
            result.append(construct_from_list([[sub]]))
    return result
